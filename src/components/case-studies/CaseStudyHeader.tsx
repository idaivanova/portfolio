    ...
    <div className={cn('flex gap-10 items-start', className)}>
      ...
      <div className="flex-1 flex flex-col gap-[30px] min-w-0">
        ...
        <div className="flex flex-col gap-2.5">
          <h1 className="font-body text-[32px] font-extrabold leading-tight text-foreground">
            {title}
          </h1>
        ...
        <div className="flex flex-wrap gap-5">
          {tags.map((tag) => (
            <span
              key={tag.label}
              className="inline-flex h-[34px] items-center justify-center rounded-sm border border-border bg-secondary/70 px-4 font-body text-sm font-bold text-accent"
            >
              {tag.label}
            </span>
          ))}
